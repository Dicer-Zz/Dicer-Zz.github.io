...
过去几年我的终端方案经历了 iTerm2 → Alacritty → WezTerm 的变迁。每次换都有一个具体的痛点：iTerm2 在大量日志输出时肉眼可见地卡顿；Alacritty 快但缺少原生 tab 和 split，必须搭配 tmux；WezTerm 用 Lua 配置很灵活，但启动速度和 GPU 渲染效率离理想状态还有距离。

2025 年底 Ghostty 1.0 发布后，我试了一下就没再换回去。同一时期 Claude Code 的出现彻底改变了我在终端里写代码的方式。现在这套方案已经稳定跑了几个月，值得记录一下。
